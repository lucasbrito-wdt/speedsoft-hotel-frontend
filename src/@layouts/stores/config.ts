...
export const useLocalStorageRef = <T>(key: string, defaultValue: T) => {
  return useLocalStorage<T>(namespaceConfig(key), defaultValue)
}
...
export const useLayoutConfigStore = defineStore('layoutConfig', () => {
  ...
  // 👉 Navbar Type
  const isNavbarBlurEnabled = useLocalStorageRef('isNavbarBlurEnabled', layoutConfig.navbar.navbarBlur)

  // 👉 Vertical Nav Collapsed
  const isVerticalNavCollapsed = useLocalStorageRef('isVerticalNavCollapsed', layoutConfig.verticalNav.isVerticalNavCollapsed)

  // 👉 App Content Width
  const appContentWidth = useLocalStorageRef('appContentWidth', layoutConfig.app.contentWidth)
